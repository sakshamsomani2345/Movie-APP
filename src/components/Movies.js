...
export default class Movies extends Component {
  ...
  async componentDidMount() {
    const res= await axios.get(`https://api.themoviedb.org/3/movie/popular?api_key=5ce2d3feb38f64f74c94208daf524e32&language=en-US&page=${this.state.currpage}`);
    const data=res.data;
this.setState({
    movies:[...data.results]
              ...
              <nav aria-label="Page navigation example">
                <ul className="pagination">
                  ...
                  {this.state.parr.map((value) => (
                    <li className="page-item">
                      <a className="page-link" onClick={()=>this.handleclick(value)}>{value}</a>
                    </li>
                  ))}
                  <li className="page-item">
